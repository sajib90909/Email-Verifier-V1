...
if($user_read3){
  ...
  if($time_left > $row3['time_range']){
    $query18 = "UPDATE timer SET last_send = '$emails', time_count = '$dates' WHERE user_id = 1 " ; // update send mail counting and time;
    ...
    if($email_left >= $emails){
      ...
      $query19 = "UPDATE timer SET last_send = '$count_email' WHERE user_id = 1 " ; // update send mail if time exits;
